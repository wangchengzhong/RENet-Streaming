...
def _create_windows_console_mouse_toggle_handler(on_toggle):
    if on_toggle is None or not hasattr(ctypes, "windll"):
        return None, "mouse toggle is only available on Windows"

    if os.environ.get("WT_SESSION"):
        return None, "Windows Terminal does not expose console mouse events to this script"
    if os.environ.get("TERM_PROGRAM", "").lower() == "vscode":
        return None, "VS Code integrated terminal does not expose console mouse events to this script"
    ...
    class KEY_EVENT_RECORD_UNION(ctypes.Union):
    ...
    if input_handle in (0, -1):
        return None, "failed to get console input handle"

    original_mode = wintypes.DWORD()
    if not kernel32.GetConsoleMode(input_handle, ctypes.byref(original_mode)):
        return None, "stdin is not a Windows console input handle"

    desired_mode = original_mode.value | ENABLE_MOUSE_INPUT | ENABLE_EXTENDED_FLAGS
    desired_mode &= ~ENABLE_QUICK_EDIT_MODE
    if not kernel32.SetConsoleMode(input_handle, desired_mode):
        return None, "failed to enable console mouse input"

    def mouse_toggle_loop(stop_event):
        record = INPUT_RECORD()
        events_read = wintypes.DWORD()
        left_down = False
        try:
            while not stop_event.is_set():
                if not kernel32.ReadConsoleInputW(input_handle, ctypes.byref(record), 1, ctypes.byref(events_read)):
                    time.sleep(0.03)
                    continue
                if events_read.value != 1 or record.EventType != MOUSE_EVENT:
                    continue

                mouse_event = record.Event.MouseEvent
                is_left_down = bool(mouse_event.dwButtonState & FROM_LEFT_1ST_BUTTON_PRESSED)
                if mouse_event.dwEventFlags == 0:
                    if is_left_down and not left_down:
                        on_toggle()
                    left_down = is_left_down
        finally:
            kernel32.SetConsoleMode(input_handle, original_mode.value)

    return mouse_toggle_loop, None
...
def main():
    ...
    parser.add_argument(
        "--enter_toggle_raw",
        action="store_true",
        help="Press Enter to toggle output between denoised and original speech",
    )
    parser.add_argument(
        "--mouse_toggle_raw",
        action="store_true",
        help="On classic Windows console hosts, left-click inside the console window to toggle output between denoised and original speech",
    )
    ...
    if (not args.bypass_model) and args.mouse_toggle_raw:
        def toggle_output_mode():
            raw_mode_state["enabled"] = not raw_mode_state["enabled"]
            mode = "ORIGINAL" if raw_mode_state["enabled"] else "DENOISED"
            print(f"output_mode={mode}")

        mouse_toggle_loop, mouse_toggle_error = _create_windows_console_mouse_toggle_handler(toggle_output_mode)
        if mouse_toggle_loop is None:
            print(f"mouse-toggle unavailable: {mouse_toggle_error}")
        else:
            print("Mouse-toggle enabled: left-click inside the console window to switch between DENOISED and ORIGINAL output.")
            mouse_toggle_thread = threading.Thread(target=mouse_toggle_loop, args=(stop_event,), daemon=True)
            mouse_toggle_thread.start()
    ...
    if mouse_toggle_thread is not None:
        mouse_toggle_thread.join(timeout=0.2)
